 ...
 *
 */
public class Actress implements Parser {
	...
	public void parse(File f) throws FileNotFoundException, SQLException, IOException, ClassNotFoundException {
		System.out.println("Creating actresses table...");
		create();
		Scanner c = new Scanner(f, "latin1");
		// skip lines that don't contain the data
		while (!c.nextLine().equals("THE ACTRESSES LIST")) {
		}
		for (int i = 0; i < 4; i++) {
		...
		while (c.hasNextLine()) {
			...
			if (line.contains("----------------------")) { // end of the file
				c.close();
				System.out.println("Creating actresses table complete!");
				return;
			}
