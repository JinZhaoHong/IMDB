 ...
 *
 */
public class Producers implements Parser {
	...
	public void parse(File f) throws FileNotFoundException, SQLException, IOException, ClassNotFoundException {
		System.out.println("Creating producers table...");
		create();
		Scanner c = new Scanner(f, "latin1");
		// skip lines that don't contain the data
		while (!c.nextLine().equals("THE PRODUCERS LIST")) {
		}
		for (int i = 0; i < 4; i++) {
		...
		while (c.hasNextLine()) {
			...
			if (line.contains("----------------------")) { // end of the file
				c.close();
				System.out.println("Creating producers table complete!");
				return;
			}
